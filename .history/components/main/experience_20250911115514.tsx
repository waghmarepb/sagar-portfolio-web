  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 bg-gradient-to-br from-[#030014] via-[#0a0a2e] to-[#16213e]">
        ...
        ></div>
        <div className="absolute top-0 left-0 w-full h-full bg-gradient-to-br from-purple-500/8 to-cyan-500/8"></div>
      </div>
      ...
      <div className="relative z-10 w-full max-w-7xl mx-auto">
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6 px-4 items-start">
            {Object.entries(experienceData.technologies).map(([category, techs], index) => (
              <motion.div
                ...
                transition={{ duration: 0.6, delay: index * 0.1 }}
                className="group relative h-48 md:h-52 w-full"
              >
                <div className="absolute inset-0 bg-gradient-to-br from-purple-500/20 to-cyan-500/20 rounded-xl blur-sm group-hover:blur-md transition-all duration-300"></div>
                <div className="relative bg-gradient-to-br from-[#0a0a1a] to-[#1a1a2e] p-4 sm:p-5 md:p-6 rounded-xl border border-purple-500/40 group-hover:border-purple-500/70 transition-all duration-300 backdrop-blur-sm h-full flex flex-col overflow-hidden shadow-lg">
                  <h3 className="text-sm sm:text-base md:text-lg font-semibold text-purple-300 mb-3 sm:mb-4 md:mb-5 capitalize flex items-center justify-center text-center">
                    <span className="w-1.5 h-1.5 sm:w-2 sm:h-2 bg-purple-400 rounded-full mr-2 sm:mr-3 flex-shrink-0"></span>
                    <span className="text-xs sm:text-sm md:text-base font-medium">{category.replace(/([A-Z])/g, ' $1').trim()}</span>
                  </h3>
                  <div className="flex flex-wrap gap-1.5 sm:gap-2 md:gap-2.5 flex-1 overflow-hidden justify-center items-start">
                    {techs.map((tech, techIndex) => (
                      <motion.span
                        ...
                        className="px-2.5 py-1.5 text-xs bg-gradient-to-r from-purple-500/30 to-cyan-500/30 text-purple-200 rounded-full border border-purple-400/40 hover:border-purple-400/70 transition-all duration-300 whitespace-nowrap font-medium text-center"
                      >
                        {tech}
                      </motion.span>
                    ))}
