          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6 px-4">
            {Object.entries(experienceData.technologies).map(([category, techs], index) => (
              ...
              >
                ...
                <div className="relative bg-gradient-to-br from-[#0a0a1a] to-[#1a1a2e] p-3 sm:p-4 md:p-6 rounded-xl border border-purple-500/40 group-hover:border-purple-500/70 transition-all duration-300 backdrop-blur-sm h-full flex flex-col overflow-hidden shadow-lg">
                  <h3 className="text-sm sm:text-base md:text-lg font-semibold text-purple-300 mb-2 sm:mb-3 md:mb-4 capitalize flex items-center">
                    <span className="w-1.5 h-1.5 sm:w-2 sm:h-2 bg-purple-400 rounded-full mr-2 sm:mr-3 flex-shrink-0"></span>
                    <span className="truncate text-xs sm:text-sm md:text-base font-medium">{category.replace(/([A-Z])/g, ' $1').trim()}</span>
                  </h3>
                  <div className="flex flex-wrap gap-1 sm:gap-1.5 md:gap-2 flex-1 overflow-hidden">
                    {techs.map((tech, techIndex) => (
                      <motion.span
                        ...
                        className="px-2 py-1 text-xs bg-gradient-to-r from-purple-500/30 to-cyan-500/30 text-purple-200 rounded-full border border-purple-400/40 hover:border-purple-400/70 transition-all duration-300 whitespace-nowrap font-medium"
                      >
                        {tech}
                      </motion.span>
                    ))}
