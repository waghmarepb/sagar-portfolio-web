  ...
  return (
    ...
    >
      ...
      <motion.div 
        ...
        className="flex flex-wrap justify-center gap-2 sm:gap-3 md:gap-4 mb-6 sm:mb-8 md:mb-12 px-4"
      >
        {projectCategories.map((category, index) => (
          <button
            key={category.name}
            onClick={() => setActiveCategory(index)}
            className={`px-3 sm:px-4 md:px-6 py-2 sm:py-2.5 md:py-3 rounded-full font-semibold transition-all duration-300 border-2 text-xs sm:text-sm md:text-base ${
      ...
      >
        <div className="text-center mb-6 sm:mb-8 px-4">
          <h2 className={`text-xl sm:text-2xl md:text-3xl font-bold bg-gradient-to-r ${projectCategories[activeCategory].color} bg-clip-text text-transparent`}>
        ...
        {/* Projects Grid */}
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-5 md:gap-6 px-4">
      ...
      <motion.div
        ...
        className="mt-12 sm:mt-16 grid grid-cols-2 md:grid-cols-4 gap-4 sm:gap-8 w-full max-w-4xl px-4"
      >
        {projectCategories.map((category, index) => (
          <div key={category.name} className="text-center">
            <div className={`text-2xl sm:text-3xl md:text-4xl font-bold bg-gradient-to-r ${category.color} bg-clip-text text-transparent`}>
              {category.projects.length}+
            </div>
            <div className="text-gray-400 text-xs sm:text-sm mt-1">{category.name} Projects</div>
          </div>
        ))}
      </motion.div>
    </section>
